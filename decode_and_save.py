...
imagenet_sample = args.imagenet_sample
...
if dim_reduction_method == 'RP':
    ...
    with open (f'reps_rps/internal_reps_rps_{model_name}_{imagenet_sample}_RPto{P}dims.pkl', 'rb') as fp:
        repDict = pickle.load(fp)
if dim_reduction_method == 'PCA':
    with open (f'reps_pca/internal_reps_pca_{model_name}_{imagenet_sample}_PCA{P*100}percent.pkl', 'rb') as fp:
        repDict = pickle.load(fp)
if dim_reduction_method is None:
    with open (f'reps/{model_name}_{imagenet_sample}.pkl', 'rb') as fp:
        repDict = pickle.load(fp)
...
# TODO:  Fix target loading!!! 

Zfull = np.load(f'targets/C_{imagenet_sample}.npy')
...
if kernel == 'linear':
    param_grid = {
        'a': [1],
        'b' : list(np.logspace(-6, 6, num=20, endpoint=True, base=10.0, dtype=None, axis=0)),
        'gamma': [1]
    }
elif kernel == 'rbf':
    param_grid = {
        'a': [1],
        'b' : list(np.logspace(-6, 6, num=10, endpoint=True, base=10.0, dtype=None, axis=0)),
        'gamma': list(np.logspace(-6, 6, num=10, endpoint=True, base=10.0, dtype=None, axis=0))
    }
...
resultDict_all["imagenet_sample"] = imagenet_sample
...
with open(f'decoding_results/color_decoding_{model_name}_{kernel}_kernel_{imagenet_sample}_RPto{P}dims.pkl', 'wb') as fp:
